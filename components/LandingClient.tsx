...
export interface LandingConfig {
  hero: { image: string; title: string; accentWord: string; subtitle: string; quote: string; quoteAuthor: string };
  pillars: { image: string; tag: string; title: string; desc: string }[];
  scenarios: { image: string; tag: string; title: string; desc: string }[];
}

/* ─── 메인 컴포넌트 ─────────────────────────────────────── */
export default function LandingClient({ config }: { config: LandingConfig | null }) {
  ...
  /* ── 서버에서 전달받은 설정 적용 (ISR, 깜빡임 없음) ── */
  const hero = config?.hero ? { ...DEFAULT_HERO, ...config.hero } : DEFAULT_HERO;
