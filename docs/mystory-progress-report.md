...
| OGP 메타태그 (공유 썸네일) | ⚠️ 부분 완료 | /shared/[id]만, 메인/my 미적용 |
...
| 항목 | 우선순위 | 설명 |
|------|---------|------|
| 책 제목/저자 편집 기능 | 높음 | 랜딩에서만 가능, 작성 중 수정 방법 없음 → /book 또는 /write에 편집 UI 필요 |
| 챕터 완성 상태 명확화 | 높음 | is_done 필드 존재하나 완성 기준 불명확 → "이야기 완성하기" 버튼 클릭 시 자동 완료 처리 |
| OGP 메타태그 전체 적용 | 중간 | /shared/[id]만 적용됨 → 랜딩 / /my / /book 페이지도 적용 |
| Resend 발신자 도메인 인증 | 중간 | 현재 onboarding@resend.dev → 자체 도메인 인증 필요 |
| OpenAI quota 관리 | 중간 | 소진 시 로컬 질문풀 폴백 중 → quota 충전 또는 사용량 모니터링 |
| 대화 모드 첫 질문 UX | 중간 | 챕터 진입 시 AI 첫 질문이 자동으로 표시되어야 하는지 명확화 필요 |
| 비회원 → 회원 전환 시 데이터 이전 | 중간 | 비회원 작성 내용을 로그인 후 계정으로 이전하는 흐름 없음 |
| 모바일 사진 업로드 UX | 낮음 | 카메라 직접 촬영 연동 (capture="environment" 속성) |
