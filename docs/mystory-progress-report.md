...
- [x] **/book 페이지 제목/저자 편집** — book/page.tsx 헤더 제목 클릭 → 인라인 편집 (제목+저자 동시)
  - setTitle/setAuthor 호출 → 2초 디바운스 자동 DB 동기화
- [x] **AI 토큰 최적화**
  - 방안 C: chat/route.ts — 최근 4개 제외한 오래된 메시지 80자로 압축
  - 방안 A: ChatEditor.tsx — 6턴 이후 3턴마다 요약, /api/ai/summarize 신규 엔드포인트
    슬라이딩 윈도우(최근 4개 전문) + 요약 컨텍스트 시스템 메시지 주입
- [x] **모바일 카메라** — ChatEditor/NormalEditor 파일 입력에 `capture="environment"` 속성 추가
- [x] **관리자 사용자 관리** — /api/admin/users (books 집계), /admin/page.tsx 탭 추가
  - 사용자별: 책 수, 공개 책 수, 첫 가입일, 마지막 활동, 작품 목록 링크
- [x] **OpenAI Quota 관리 대시보드** — /api/admin/openai-usage 신규
  - API 키 유효성, 이번 달 사용 비용, 월 한도, 관리 링크 카드
...
| OpenAI quota 관리 | 중간 | ✅ 완료 | 관리자 대시보드 OpenAI 상태 카드 추가 (Phase 11) |
| 모바일 사진 업로드 UX | 낮음 | ✅ 완료 | capture="environment" 속성 추가 (Phase 11) |
| /book 페이지 내 제목/저자 편집 | 낮음 | ✅ 완료 | 책 미리보기 헤더 클릭 인라인 편집 (Phase 11) |
